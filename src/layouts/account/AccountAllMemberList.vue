            ...
            <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="user-name font-medium truncate">
                  ...
                  <vs-checkbox v-model="tr.marketing" @click="changeAccType(tr)"
                    >Marketing</vs-checkbox
                  >
                </p>
                <p>
                  <vs-button
          ...
          for (let obj in data) {
            ...
            for (let i = 0; i < data[obj].length; i++) {
              this.treeData[0].children.push({
                title: `<span style='color: white'>Cấp ${c} - ${data[obj][i].nick_name} - Khối lượng Giao Dịch: ${this.formatPrice(data[obj][i].tklgd, 2)}</span> - Tổng tiền nạp: ${this.formatPrice(data[obj][i].amt, 2)}`,
              });
            }
          }
        }
