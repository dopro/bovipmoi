<template>
  <div class="footerFAQs">
    <div class="container position-relative">
      <div class="py-3 py-sm-5 blockRow border-bottom pb-0">
        <div class="block logo">
          <p class="mb-lg-2 mb-1">
            <img height="73" src="https://i.ibb.co/D9Q7pFC/logo3.png" alt="" />
          </p>
          <div class="font-12 txtContent">
            © 2024 FINOECH All rights reserved.
          </div>
          <p>
            Email:
            <a href="mailto:admin@finoech.net">admin@finoech.net</a>
          </p>
        </div>
        <div class="block pl-sm-4">
          <h4>What we do</h4>
          <ul>
            <li>
              <p>
                <a href="./aboutUs.html" target="_blank">About us</a>
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs" class="txtContent">Digital Asset Exchange</a>
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs" class="txtContent">General Provisions</a>
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs" class="txtContent">Terms of use</a>
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs" class="txtContent">Rights and Obligations</a>
              </p>
            </li>
          </ul>
        </div>
        <div class="block pl-sm-4" style="flex: 0.5 1 0%">
          <h4>Support</h4>
          <ul>
            <li>
              <p>
                <a href="/faqs" class="">FAQs</a>
              </p>
            </li>
            <li>
              <p>
                <a href="#" download="agencyObligations.pdf" target="_blank"
                  >VIP Obligations</a
                >
              </p>
            </li>
            <li>
              <p>
                <a href="./aboutUs.html" target="_blank">Communications</a>
              </p>
            </li>
            <li>
              <p>
                <a href="./aboutUs.html" target="_blank">Dispute Resolution</a>
              </p>
            </li>
            <li>
              <p>
                <a href="./aboutUs.html" target="_blank">Governing Law</a>
              </p>
            </li>
          </ul>
        </div>
        <div class="block pl-sm-4">
          <h4>Security &amp; Privacy</h4>
          <ul>
            <li>
              <p>
                <a href="/faqs/terms-of-use" class="txtContent"
                  >Terms and Conditions</a
                >
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs/risks-disclosure" class="txtContent"
                  >Risks Disclosure</a
                >
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs/liability-and-indemnity" class="txtContent"
                  >Liability and Indemnity</a
                >
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs/liability-and-indemnity" class="txtContent"
                  >Force Majeure</a
                >
              </p>
            </li>
            <li>
              <p>
                <a href="/faqs/liability-and-indemnity" class="txtContent"
                  >Liability of the Parties</a
                >
              </p>
            </li>
          </ul>
        </div>
        <div class="block pl-sm-4">
          <div class="dropdown language">
            <button
              type="button"
              id="dropdownMenuButton"
              data-toggle="dropdown"
              aria-haspopup="true"
              aria-expanded="false"
              class="btn dropdown-toggle colorSecondary dropdownMenuButton bgPrimary1 position-relative"
            >
              <img
                width="18"
                src="data:image/png;base64,"
                alt=""
                class="mr-2 mb-1"
              /><span class="colorSecondary">English</span>
            </button>
            <!-- <div aria-labelledby="dropdownMenuButton" class="dropdown-menu">
                <div class="dropdown-item pointer" style="display: none;">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>English</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>Tiếng Việt</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>日本</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="/landing/img/kr.27eb9a27.png"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>한국어</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>中国</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>ប្រទេសកម្ពុជា</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>Thai</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>Indonesian</span>
                </div>
                <div class="dropdown-item pointer">
                  <img
                    width="18"
                    src="data:image/png;base64,"
                    alt=""
                    class="mr-2 mb-1"
                  /><span>Lao</span>
                </div>
              </div> -->
          </div>
        </div>
      </div>
      <div class="ft-1">
        <div class="block">
          <p class="des1">
            fionech is authorised by the fionech TRADING LIMITED and
            regulated under the Judicial Authority of Portugal. fionech
            TRADING LIMITED (125,268) is incorporated in Portugal and has our
            registered office at: R. Prof. Lima Basto 1099-023, Lisbon,
            Portugal.
          </p>
          <br />
          <p style="text-align: center" class="des2">
            © fionech {{ new Date().getFullYear() }} All rights reserved.
            ’fionech’ is a trademark of the fionech TRADING LIMITED.
          </p>
        </div>
      </div>
    </div>
  </div>
</template>

<style lang="scss" scoped>
.footerFAQs {
  padding-top: 40px;
  display: block;
  color: #fff;
  background: #19191b;

  .container {
    position: relative;
    max-width: 1140px;
    width: 100%;
    padding-right: 15px;
    padding-left: 15px;
    margin-right: auto;
    margin-left: auto;
  }

  .blockRow {
    display: flex;
    border-color: rgba(139, 141, 150, 0.28);
    padding: 3rem 0 !important;
  }

  .block {
    flex: 1;
    padding-right: 20px;
    position: relative;

    h4 {
      font-size: 16px;
      margin-bottom: 20px;
      margin-top: 8px;
      color: #fff;
    }

    li p {
      margin-bottom: 10px;
      a {
        color: #fff;
        opacity: 0.6;
        line-height: 30px;
      }
    }

    .language {
      display: flex;
      justify-content: flex-end;
      align-content: center;
      button {
        display: inline-block;
        font-weight: 400;
        text-align: center;
        vertical-align: middle;
        user-select: none;
        border: 1px solid transparent;
        padding: 0.8rem 0.75rem;
        border-radius: 0.25rem;
        color: #fff;
        background: #161718;
        display: flex;
        align-content: center;
      }
      span {
        color: #fff;
      }
    }
  }

  .logo {
    margin-bottom: 0.5rem;
    margin-right: 2rem;
    img {
      max-width: 100%;
      font-style: italic;
      vertical-align: middle;
    }
    a {
      color: #fa2843;
      text-decoration: none;
    }
  }

  .ft-1 {
    padding: 3rem 0;
  }

  .des1,
  .des2 {
    margin-bottom: 1rem;
  }
}
@media (max-width: 576px) {
  .footerFAQs {
    .blockRow {
      display: unset;
    }

    .logo p img {
      width: 100%;
      object-fit: cover;
      margin-bottom: 10px;
    }

    .block {
      flex: unset;
      .language {
        justify-content: flex-start !important;
        margin-top: 20px;
      }
    }
  }
  .footerFAQs {
    .blockRow {
      display: unset;
    }

    .logo p img {
      width: 100%;
      object-fit: cover;
      margin-bottom: 10px;
    }

    .block {
      flex: unset;
      .language {
        justify-content: flex-start !important;
        margin-top: 20px;
      }
    }
  }
}
</style>

<script>
export default {};
</script>
