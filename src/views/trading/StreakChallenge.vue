<template>
  <div id="main-content">
    <div data-v-59d426a6="" id="community-jackpot" class="community-jackpot">
      <h1 class="community-jackpot-title">Streak Challenge</h1>
      <div class="prize-pool">
        <h3 class="prize-pool-label"><span>Prize Pool</span></h3>
        <p class="prize-pool-value mb-0">$33,680.35</p>
        <span class="prize-pool-mega-fund">$168.4</span>
      </div>
      <a href="/trading" class="nav-link go-trade">Giao dịch và Thắng Giải</a>
      <p class="link-to-info mb-0">Xem thêm thông tin</p>
      <div class="bg">
        <div data-v-d6d779c0="" class="latest-winner">
          <div data-v-d6d779c0="" class="container">
            <h3 data-v-d6d779c0="" class="latest-winner-subtitle">
              Danh sách người nhận giải
            </h3>
            <h2 data-v-d6d779c0="" class="latest-winner-title">
              Latest Winners
            </h2>
            <div data-v-d6d779c0="" class="latest-winner-mega">
              <div class="loading">
                <div class="loading__ring">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    version="1.1"
                    x="0px"
                    y="0px"
                    viewBox="0 0 100 100"
                    style="enable-background: new 0 0 100 100"
                    xml:space="preserve"
                  >
                    <path
                      d="M85.5,42c-0.2-0.8-0.5-1.7-0.8-2.5c-0.3-0.9-0.7-1.6-1-2.3c-0.3-0.7-0.6-1.3-1-1.9c0.3,0.5,0.5,1.1,0.8,1.7  c0.2,0.7,0.6,1.5,0.8,2.3s0.5,1.7,0.8,2.5c0.8,3.5,1.3,7.5,0.8,12c-0.4,4.3-1.8,9-4.2,13.4c-2.4,4.2-5.9,8.2-10.5,11.2  c-1.1,0.7-2.2,1.5-3.4,2c-0.5,0.2-1.2,0.6-1.8,0.8s-1.3,0.5-1.9,0.8c-2.6,1-5.3,1.7-8.1,1.8l-1.1,0.1L53.8,84c-0.7,0-1.4,0-2.1,0  c-1.4-0.1-2.9-0.1-4.2-0.5c-1.4-0.1-2.8-0.6-4.1-0.8c-1.4-0.5-2.7-0.9-3.9-1.5c-1.2-0.6-2.4-1.2-3.7-1.9c-0.6-0.3-1.2-0.7-1.7-1.1  l-0.8-0.6c-0.3-0.1-0.6-0.4-0.8-0.6l-0.8-0.6L31.3,76l-0.2-0.2L31,75.7l-0.1-0.1l0,0l-1.5-1.5c-1.2-1-1.9-2.1-2.7-3.1  c-0.4-0.4-0.7-1.1-1.1-1.7l-1.1-1.7c-0.3-0.6-0.6-1.2-0.9-1.8c-0.2-0.5-0.6-1.2-0.8-1.8c-0.4-1.2-1-2.4-1.2-3.7  c-0.2-0.6-0.4-1.2-0.5-1.9c-0.1-0.6-0.2-1.2-0.3-1.8c-0.3-1.2-0.3-2.4-0.4-3.7c-0.1-1.2,0-2.5,0.1-3.7c0.2-1.2,0.3-2.4,0.6-3.5  c0.1-0.6,0.3-1.1,0.4-1.7l0.1-0.8l0.3-0.8c1.5-4.3,3.8-8,6.5-11c0.8-0.8,1.4-1.5,2.1-2.1c0.9-0.9,1.4-1.3,2.2-1.8  c1.4-1.2,2.9-2,4.3-2.8c2.8-1.5,5.5-2.3,7.7-2.8s4-0.7,5.2-0.6c0.6-0.1,1.1,0,1.4,0s0.4,0,0.4,0h0.1c2.7,0.1,5-2.2,5-5  c0.1-2.7-2.2-5-5-5c-0.2,0-0.2,0-0.3,0c0,0-0.2,0.1-0.6,0.1c-0.4,0-1,0-1.8,0.1c-1.6,0.1-4,0.4-6.9,1.2c-2.9,0.8-6.4,2-9.9,4.1  c-1.8,1-3.6,2.3-5.4,3.8C26,21.4,25,22.2,24.4,23c-0.2,0.2-0.4,0.4-0.6,0.6c-0.2,0.2-0.5,0.4-0.6,0.7c-0.5,0.4-0.8,0.9-1.3,1.4  c-3.2,3.9-5.9,8.8-7.5,14.3l-0.3,1l-0.2,1.1c-0.1,0.7-0.3,1.4-0.4,2.1c-0.3,1.5-0.4,2.9-0.5,4.5c0,1.5-0.1,3,0.1,4.5  c0.2,1.5,0.2,3,0.6,4.6c0.1,0.7,0.3,1.5,0.4,2.3c0.2,0.8,0.5,1.5,0.7,2.3c0.4,1.6,1.1,3,1.7,4.4c0.3,0.7,0.7,1.4,1.1,2.1  c0.4,0.8,0.8,1.4,1.2,2.1c0.5,0.7,0.9,1.4,1.4,2s0.9,1.3,1.5,1.9c1.1,1.3,2.2,2.7,3.3,3.5l1.7,1.6c0,0,0.1,0.1,0.1,0.1c0,0,0,0,0,0  c0,0,0,0,0,0l0.1,0.1l0.1,0.1h0.2l0.5,0.4l1,0.7c0.4,0.2,0.6,0.5,1,0.7l1.1,0.6c0.8,0.4,1.4,0.9,2.1,1.2c1.4,0.7,2.9,1.5,4.4,2  c1.5,0.7,3.1,1,4.6,1.5c1.5,0.3,3.1,0.7,4.7,0.8c1.6,0.2,3.1,0.2,4.7,0.2c0.8,0,1.6-0.1,2.4-0.1l1.2-0.1l1.1-0.1  c3.1-0.4,6.1-1.3,8.9-2.4c0.8-0.3,1.4-0.6,2.1-0.9s1.3-0.7,2-1.1c1.3-0.7,2.6-1.7,3.7-2.5c0.5-0.4,1-0.9,1.6-1.3l0.8-0.6l0.2-0.2  c0,0,0.1-0.1,0.1-0.1c0.1-0.1,0,0,0,0v0.1l0.1-0.1l0.4-0.4c0.5-0.5,1-1,1.5-1.5c0.3-0.3,0.5-0.5,0.8-0.8l0.7-0.8  c0.9-1.1,1.8-2.2,2.5-3.3c0.4-0.6,0.7-1.1,1.1-1.7c0.3-0.7,0.6-1.2,0.9-1.8c2.4-4.9,3.5-9.8,3.7-14.4C87.3,49.7,86.6,45.5,85.5,42z"
                    ></path>
                  </svg>
                </div>
                <div class="loading__ring">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    version="1.1"
                    x="0px"
                    y="0px"
                    viewBox="0 0 100 100"
                    style="enable-background: new 0 0 100 100"
                    xml:space="preserve"
                  >
                    <path
                      d="M85.5,42c-0.2-0.8-0.5-1.7-0.8-2.5c-0.3-0.9-0.7-1.6-1-2.3c-0.3-0.7-0.6-1.3-1-1.9c0.3,0.5,0.5,1.1,0.8,1.7  c0.2,0.7,0.6,1.5,0.8,2.3s0.5,1.7,0.8,2.5c0.8,3.5,1.3,7.5,0.8,12c-0.4,4.3-1.8,9-4.2,13.4c-2.4,4.2-5.9,8.2-10.5,11.2  c-1.1,0.7-2.2,1.5-3.4,2c-0.5,0.2-1.2,0.6-1.8,0.8s-1.3,0.5-1.9,0.8c-2.6,1-5.3,1.7-8.1,1.8l-1.1,0.1L53.8,84c-0.7,0-1.4,0-2.1,0  c-1.4-0.1-2.9-0.1-4.2-0.5c-1.4-0.1-2.8-0.6-4.1-0.8c-1.4-0.5-2.7-0.9-3.9-1.5c-1.2-0.6-2.4-1.2-3.7-1.9c-0.6-0.3-1.2-0.7-1.7-1.1  l-0.8-0.6c-0.3-0.1-0.6-0.4-0.8-0.6l-0.8-0.6L31.3,76l-0.2-0.2L31,75.7l-0.1-0.1l0,0l-1.5-1.5c-1.2-1-1.9-2.1-2.7-3.1  c-0.4-0.4-0.7-1.1-1.1-1.7l-1.1-1.7c-0.3-0.6-0.6-1.2-0.9-1.8c-0.2-0.5-0.6-1.2-0.8-1.8c-0.4-1.2-1-2.4-1.2-3.7  c-0.2-0.6-0.4-1.2-0.5-1.9c-0.1-0.6-0.2-1.2-0.3-1.8c-0.3-1.2-0.3-2.4-0.4-3.7c-0.1-1.2,0-2.5,0.1-3.7c0.2-1.2,0.3-2.4,0.6-3.5  c0.1-0.6,0.3-1.1,0.4-1.7l0.1-0.8l0.3-0.8c1.5-4.3,3.8-8,6.5-11c0.8-0.8,1.4-1.5,2.1-2.1c0.9-0.9,1.4-1.3,2.2-1.8  c1.4-1.2,2.9-2,4.3-2.8c2.8-1.5,5.5-2.3,7.7-2.8s4-0.7,5.2-0.6c0.6-0.1,1.1,0,1.4,0s0.4,0,0.4,0h0.1c2.7,0.1,5-2.2,5-5  c0.1-2.7-2.2-5-5-5c-0.2,0-0.2,0-0.3,0c0,0-0.2,0.1-0.6,0.1c-0.4,0-1,0-1.8,0.1c-1.6,0.1-4,0.4-6.9,1.2c-2.9,0.8-6.4,2-9.9,4.1  c-1.8,1-3.6,2.3-5.4,3.8C26,21.4,25,22.2,24.4,23c-0.2,0.2-0.4,0.4-0.6,0.6c-0.2,0.2-0.5,0.4-0.6,0.7c-0.5,0.4-0.8,0.9-1.3,1.4  c-3.2,3.9-5.9,8.8-7.5,14.3l-0.3,1l-0.2,1.1c-0.1,0.7-0.3,1.4-0.4,2.1c-0.3,1.5-0.4,2.9-0.5,4.5c0,1.5-0.1,3,0.1,4.5  c0.2,1.5,0.2,3,0.6,4.6c0.1,0.7,0.3,1.5,0.4,2.3c0.2,0.8,0.5,1.5,0.7,2.3c0.4,1.6,1.1,3,1.7,4.4c0.3,0.7,0.7,1.4,1.1,2.1  c0.4,0.8,0.8,1.4,1.2,2.1c0.5,0.7,0.9,1.4,1.4,2s0.9,1.3,1.5,1.9c1.1,1.3,2.2,2.7,3.3,3.5l1.7,1.6c0,0,0.1,0.1,0.1,0.1c0,0,0,0,0,0  c0,0,0,0,0,0l0.1,0.1l0.1,0.1h0.2l0.5,0.4l1,0.7c0.4,0.2,0.6,0.5,1,0.7l1.1,0.6c0.8,0.4,1.4,0.9,2.1,1.2c1.4,0.7,2.9,1.5,4.4,2  c1.5,0.7,3.1,1,4.6,1.5c1.5,0.3,3.1,0.7,4.7,0.8c1.6,0.2,3.1,0.2,4.7,0.2c0.8,0,1.6-0.1,2.4-0.1l1.2-0.1l1.1-0.1  c3.1-0.4,6.1-1.3,8.9-2.4c0.8-0.3,1.4-0.6,2.1-0.9s1.3-0.7,2-1.1c1.3-0.7,2.6-1.7,3.7-2.5c0.5-0.4,1-0.9,1.6-1.3l0.8-0.6l0.2-0.2  c0,0,0.1-0.1,0.1-0.1c0.1-0.1,0,0,0,0v0.1l0.1-0.1l0.4-0.4c0.5-0.5,1-1,1.5-1.5c0.3-0.3,0.5-0.5,0.8-0.8l0.7-0.8  c0.9-1.1,1.8-2.2,2.5-3.3c0.4-0.6,0.7-1.1,1.1-1.7c0.3-0.7,0.6-1.2,0.9-1.8c2.4-4.9,3.5-9.8,3.7-14.4C87.3,49.7,86.6,45.5,85.5,42z"
                    ></path>
                  </svg>
                </div>
              </div>
              <h4 data-v-d6d779c0="" class="latest-winner-mega-title">
                mega jackpot winner
              </h4>
              <div data-v-d6d779c0="" class="latest-winner-mega-body">
                <p data-v-d6d779c0="" class="latest-winner-mega-name">wyn***</p>
                <p data-v-d6d779c0="" class="latest-winner-mega-text">
                  Won Mega Prizes 09/18/2022
                </p>
              </div>
              <span data-v-d6d779c0="" class="latest-winner-mega-prize"
                >$170.63</span
              >
            </div>
            <div data-v-d6d779c0="" class="latest-winner-table">
              <ul data-v-d6d779c0="" class="nav nav-tabs">
                <li data-v-d6d779c0="" class="item active">
                  Lịch sử nhận giải
                </li>
                <li data-v-d6d779c0="" class="item">Lịch sử của bạn</li>
              </ul>
              <table class="table-winner">
                <tr class="latest-winner-item latest-winner-item--header">
                  <th>Thời gian</th>
                  <th>Biệt danh</th>
                  <th>Streak</th>
                  <th>Phần thưởng</th>
                </tr>
                <tr class="latest-winner-items">
                  <td>09/20/2022</td>
                  <td>boy***</td>
                  <td class="win">Win Streak 9x</td>
                  <td>$33.44</td>
                </tr>
                <tr class="latest-winner-items">
                  <td>09/20/2022</td>
                  <td>boy***</td>
                  <td class="loss">Win Streak 9x</td>
                  <td>$33.44</td>
                </tr>
                <tr class="latest-winner-items">
                  <td>09/20/2022</td>
                  <td>boy***</td>
                  <td class="win">Win Streak 9x</td>
                  <td>$33.44</td>
                </tr>
                <tr class="latest-winner-items">
                  <td>09/20/2022</td>
                  <td>boy***</td>
                  <td class="win">Win Streak 9x</td>
                  <td>$33.44</td>
                </tr>
                <tr class="latest-winner-items">
                  <td>09/20/2022</td>
                  <td>boy***</td>
                  <td class="win">Win Streak 9x</td>
                  <td>$33.44</td>
                </tr>
              </table>
            </div>
            <div data-v-d6d779c0="" class="d-flex justify-content-center pt-8">
              <div
                data-v-6169587e=""
                data-v-d6d779c0=""
                id="paging"
                class="paging text-center mb-4"
              >
                <ul data-v-6169587e="" id="page" class="list-unstyled mb-0">
                  <!---->
                  <!---->
                  <!---->
                  <li data-v-6169587e="">
                    <a
                      data-v-6169587e=""
                      href="javascript:void(0)"
                      data-page="1"
                      class="active btn-active-paging"
                      >1</a
                    >
                  </li>
                  <li data-v-6169587e="">
                    <a data-v-6169587e="" href="?page=2" data-page="2">2</a>
                  </li>
                  <li data-v-6169587e="">
                    <a data-v-6169587e="" href="?page=3" data-page="3">3</a>
                  </li>
                  <li data-v-6169587e="" class="pc">
                    <a
                      data-v-6169587e=""
                      href="javascript:void(0);"
                      class="text"
                      >...</a
                    >
                  </li>
                  <li data-v-6169587e="" class="pc">
                    <a data-v-6169587e="" href="?page=5025">5025</a>
                  </li>
                  <li data-v-6169587e="">
                    <a data-v-6169587e="" href="?page=2" data-page="2">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink"
                        version="1.1"
                        id="Capa_1"
                        x="0px"
                        y="0px"
                        viewBox="0 0 59.414 59.414"
                        style="enable-background: new 0 0 59.414 59.414"
                        xml:space="preserve"
                      >
                        <g>
                          <polygon
                            style="fill: #48a0dc"
                            points="15.561,59.414 14.146,58 42.439,29.707 14.146,1.414 15.561,0 45.268,29.707  "
                          />
                        </g>
                      </svg>
                    </a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
        <div id="info-guide" class="info-guide">
          <div class="container info-guide-inner">
            <div class="info-guide-header">
              <h3 class="info-guide-subtitle">Guidelines</h3>
              <h2 class="info-guide-title">How To Join Challenge</h2>
            </div>
            <div class="info-guide-body">
              <div class="info-guide-slider">
                <div
                  class="swiper-container swiper-community-jackpot swiper-container-horizontal swiper-container-ios"
                >
                  <div
                    class="swiper-wrapper"
                    style="
                      transition-duration: 0ms;
                      transform: translate3d(0px, 0px, 0px);
                    "
                  >
                    <div class="swiper-slide swiper-slide-active">
                      <div class="swiper-slide-inner">
                        <div class="svg-winner">
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="79.343"
                            height="84.019"
                            viewBox="0 0 79.343 84.019"
                            class="swiper-slide-icon"
                          >
                            <g
                              id="Group_21270"
                              data-name="Group 21270"
                              transform="translate(-354.969 -2485.583)"
                            >
                              <g
                                id="页面1"
                                transform="translate(361.869 2487.832)"
                              >
                                <g
                                  id="_1_未登录"
                                  data-name="1_未登录"
                                  transform="translate(-5.258 -2.249)"
                                >
                                  <g id="编组-7" transform="translate(0 0)">
                                    <g id="编组备份" transform="translate(0 0)">
                                      <g
                                        id="Group-6"
                                        transform="translate(0 0)"
                                      >
                                        <g id="分组" transform="translate(0 0)">
                                          <path
                                            id="合并形状-copy-2"
                                            d="M2.553,30.776a24.9,24.9,0,0,0,6.33.813A25.189,25.189,0,0,0,33.953,6.282V3.848h2.435A38.337,38.337,0,1,1-1.642,42.183,38.688,38.688,0,0,1-.424,32.524l.61-2.369Z"
                                            transform="translate(1.642 1.859)"
                                            fill="#fff"
                                            stroke="#fff"
                                            stroke-width="3.283"
                                            fill-rule="evenodd"
                                          ></path>
                                          <path
                                            id="_1"
                                            data-name="1"
                                            d="M14.653,0l-4.2,22.419H6.782L10.155,4.471,5.361,8.155l.829-4.5L10.984,0Z"
                                            transform="translate(5.025 0)"
                                            fill="#fff"
                                          ></path>
                                        </g>
                                      </g>
                                    </g>
                                  </g>
                                </g>
                              </g>
                              <g id="skill" transform="translate(370 2512.416)">
                                <path
                                  id="Path_35025"
                                  data-name="Path 35025"
                                  d="M10.275,5.284c9.074,0,15.306-4.7,19.678-4.7S43.509,3.1,43.509,23.213,31.155,46.279,26.346,46.279C3.825,46.281-5.8,5.284,10.275,5.284Z"
                                  transform="translate(0.09)"
                                  fill="#efefef"
                                ></path>
                                <g
                                  id="Group_21244"
                                  data-name="Group 21244"
                                  transform="translate(33.355 24.623)"
                                >
                                  <path
                                    id="Path_35026"
                                    data-name="Path 35026"
                                    d="M0,0H2.624V1.312H0Z"
                                    transform="translate(5.335 1.855) rotate(-45)"
                                    fill="#a4afc1"
                                  ></path>
                                  <path
                                    id="Path_35027"
                                    data-name="Path 35027"
                                    d="M0,0H2.624V1.312H0Z"
                                    transform="translate(0 7.188) rotate(-45)"
                                    fill="#a4afc1"
                                  ></path>
                                  <path
                                    id="Path_35028"
                                    data-name="Path 35028"
                                    d="M0,0H1.312V2.624H0Z"
                                    transform="translate(5.333 6.02) rotate(-45)"
                                    fill="#a4afc1"
                                  ></path>
                                </g>
                                <path
                                  id="Path_35029"
                                  data-name="Path 35029"
                                  d="M16.755,8.062A1.316,1.316,0,0,1,18.067,6.75h2.65a2.624,2.624,0,1,0,4.539,0h2.65a1.316,1.316,0,0,1,1.312,1.312v2.65a2.624,2.624,0,1,1,0,4.539V17.9a1.316,1.316,0,0,1-1.312,1.312H18.045A1.312,1.312,0,0,1,16.733,17.9Z"
                                  transform="translate(4.595 1.923)"
                                  fill="#2fdf84"
                                ></path>
                                <circle
                                  id="Ellipse_2092"
                                  data-name="Ellipse 2092"
                                  cx="4.264"
                                  cy="4.264"
                                  r="4.264"
                                  transform="translate(13.479 16.544)"
                                  fill="#f3f3f1"
                                ></circle>
                                <path
                                  id="Path_35030"
                                  data-name="Path 35030"
                                  d="M25.772,32.261V28a5.246,5.246,0,0,0-5.248-5.248H12A5.246,5.246,0,0,0,6.75,28v4.264Z"
                                  transform="translate(1.481 6.913)"
                                  fill="#f3f3f1"
                                ></path>
                                <path
                                  id="Path_35031"
                                  data-name="Path 35031"
                                  d="M19.685,17.9l.022-9.835a1.3,1.3,0,0,1,.983-1.245c.012-.021.016-.046.028-.067h-2.65a1.316,1.316,0,0,0-1.312,1.312L16.733,17.9a1.311,1.311,0,0,0,1.312,1.316H21A1.312,1.312,0,0,1,19.685,17.9Z"
                                  transform="translate(4.595 1.923)"
                                  fill="#00b871"
                                ></path>
                                <path
                                  id="Path_35032"
                                  data-name="Path 35032"
                                  d="M13.7,17.014a4.253,4.253,0,0,1,2.788-3.985,4.264,4.264,0,1,0,0,7.971A4.253,4.253,0,0,1,13.7,17.014Z"
                                  transform="translate(2.729 3.794)"
                                  fill="#d5dbe1"
                                ></path>
                                <path
                                  id="Path_35033"
                                  data-name="Path 35033"
                                  d="M14.949,22.75H12A5.246,5.246,0,0,0,6.75,28v4.264H9.7V28A5.246,5.246,0,0,1,14.949,22.75Z"
                                  transform="translate(1.481 6.913)"
                                  fill="#d5dbe1"
                                ></path>
                                <path
                                  id="Path_35034"
                                  data-name="Path 35034"
                                  d="M28.135,20.431H21.221V18.463h6.914a.332.332,0,0,0,.328-.328v-2.65a.983.983,0,0,1,1.481-.849,1.64,1.64,0,1,0,0-2.842.983.983,0,0,1-1.481-.849V8.3a.332.332,0,0,0-.328-.328H26.807c.01.109.016.219.016.328a3.608,3.608,0,1,1-7.215,0c0-.109.005-.219.016-.328H18.3a.332.332,0,0,0-.328.328V11.9H16V8.3A2.3,2.3,0,0,1,18.3,6h2.65a.984.984,0,0,1,.849,1.481,1.64,1.64,0,1,0,2.842,0A.984.984,0,0,1,25.485,6h2.65a2.3,2.3,0,0,1,2.3,2.3V9.622a3.608,3.608,0,1,1,0,7.186v1.326A2.3,2.3,0,0,1,28.135,20.431Z"
                                  transform="translate(4.366 1.689)"
                                ></path>
                                <path
                                  id="Path_35035"
                                  data-name="Path 35035"
                                  d="M15.248,22.5A5.248,5.248,0,1,1,20.5,17.248,5.253,5.253,0,0,1,15.248,22.5Zm0-8.527a3.28,3.28,0,1,0,3.28,3.28A3.283,3.283,0,0,0,15.248,13.968Z"
                                  transform="translate(2.495 3.56)"
                                ></path>
                                <path
                                  id="Path_35036"
                                  data-name="Path 35036"
                                  d="M26.99,32.5H25.022V28.231a4.269,4.269,0,0,0-4.264-4.264H12.231a4.269,4.269,0,0,0-4.264,4.264V32.5H6V28.231A6.239,6.239,0,0,1,12.231,22h8.527a6.239,6.239,0,0,1,6.231,6.231Z"
                                  transform="translate(1.248 6.679)"
                                ></path>
                                <path
                                  id="Path_35037"
                                  data-name="Path 35037"
                                  d="M2,28.5H43.98v1.968H2Z"
                                  transform="translate(0 8.706)"
                                ></path>
                              </g>
                            </g>
                          </svg>
                        </div>
                        <div class="flex flex-col">
                          <span class="swiper-slide-label">Đăng ký</span
                          ><span class="swiper-slide-text"
                            >Đăng ký vista3.trade</span
                          >
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide swiper-slide-next">
                      <div class="swiper-slide-inner">
                        <div class="svg-winner">
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="81.189"
                            height="86.019"
                            viewBox="0 0 81.189 86.019"
                            class="swiper-slide-icon"
                          >
                            <g
                              id="Group_21271"
                              data-name="Group 21271"
                              transform="translate(-555.047 -2483.583)"
                            >
                              <g
                                id="页面1"
                                transform="translate(554.33 2483.583)"
                              >
                                <g
                                  id="_1_未登录"
                                  data-name="1_未登录"
                                  transform="translate(2.358 0)"
                                >
                                  <g id="编组-7" transform="translate(0)">
                                    <g id="编组备份">
                                      <g id="Group-6-Copy-3">
                                        <g id="分组-2">
                                          <path
                                            id="合并形状-copy"
                                            d="M2.654,31.43a25.5,25.5,0,0,0,6.484.833A25.8,25.8,0,0,0,34.817,6.341V3.848h2.494A39.267,39.267,0,1,1-1.642,43.114,39.627,39.627,0,0,1-.394,33.22l.625-2.426Z"
                                            transform="translate(1.642 1.997)"
                                            fill="#fff"
                                            stroke="#fff"
                                            stroke-width="3.283"
                                            fill-rule="evenodd"
                                          ></path>
                                          <path
                                            id="_2"
                                            data-name="2"
                                            d="M19.806,5.661c0,3.1-2.069,4.957-3.355,6.3L9.3,19.413h8.215l-.721,3.55H3.627l.721-3.55L14.005,9.4c1.286-1.343,1.881-2.175,1.881-3.55a2.022,2.022,0,0,0-2.132-2.3c-1.411,0-2.728.768-3.23,2.974H6.605C7.358,2.175,10.086,0,14.162,0,17.643,0,19.806,2.047,19.806,5.661Z"
                                            transform="translate(4.377)"
                                            fill="#fff"
                                          ></path>
                                        </g>
                                      </g>
                                    </g>
                                  </g>
                                </g>
                              </g>
                              <g
                                id="increase"
                                transform="translate(571.916 2511.971)"
                              >
                                <g
                                  id="Group_21245"
                                  data-name="Group 21245"
                                  transform="translate(2.384 0.583)"
                                >
                                  <path
                                    id="Path_35038"
                                    data-name="Path 35038"
                                    d="M35.847,41.978c-9.163,0-15.455,4.746-19.87,4.746S2.29,44.186,2.29,23.876,14.764.583,19.62.583C42.361.583,52.074,41.978,35.847,41.978Z"
                                    transform="translate(-2.29 -0.583)"
                                    fill="#efefef"
                                    opacity="0.715"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21246"
                                  data-name="Group 21246"
                                  transform="translate(35.116 3.785)"
                                >
                                  <path
                                    id="Path_35039"
                                    data-name="Path 35039"
                                    d="M28.987,6.974a1.987,1.987,0,1,1,1.987-1.987A1.989,1.989,0,0,1,28.987,6.974Zm0-2.649a.662.662,0,1,0,.662.662A.663.663,0,0,0,28.987,4.325Z"
                                    transform="translate(-27 -3)"
                                    fill="#a4afc1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21247"
                                  data-name="Group 21247"
                                  transform="translate(9.617 27.297)"
                                >
                                  <path
                                    id="Path_35040"
                                    data-name="Path 35040"
                                    d="M13.711,32.009H7.75v-10.6a.663.663,0,0,1,.662-.662h4.636a.663.663,0,0,1,.662.662Z"
                                    transform="translate(-7.75 -20.75)"
                                    fill="#f3f3f1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21248"
                                  data-name="Group 21248"
                                  transform="translate(20.214 21.999)"
                                >
                                  <path
                                    id="Path_35041"
                                    data-name="Path 35041"
                                    d="M21.711,33.308H15.75v-15.9a.663.663,0,0,1,.662-.662h4.636a.663.663,0,0,1,.662.662Z"
                                    transform="translate(-15.75 -16.75)"
                                    fill="#f3f3f1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21249"
                                  data-name="Group 21249"
                                  transform="translate(30.811 16.7)"
                                >
                                  <path
                                    id="Path_35042"
                                    data-name="Path 35042"
                                    d="M29.711,34.607H23.75V13.412a.663.663,0,0,1,.662-.662h4.636a.663.663,0,0,1,.662.662Z"
                                    transform="translate(-23.75 -12.75)"
                                    fill="#2fdf84"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21250"
                                  data-name="Group 21250"
                                  transform="translate(9.617 27.297)"
                                >
                                  <path
                                    id="Path_35043"
                                    data-name="Path 35043"
                                    d="M11.393,20.75H8.412a.663.663,0,0,0-.662.662v10.6h2.98v-10.6A.663.663,0,0,1,11.393,20.75Z"
                                    transform="translate(-7.75 -20.75)"
                                    fill="#d5dbe1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21251"
                                  data-name="Group 21251"
                                  transform="translate(20.214 21.999)"
                                >
                                  <path
                                    id="Path_35044"
                                    data-name="Path 35044"
                                    d="M19.393,16.75h-2.98a.663.663,0,0,0-.662.662v15.9h2.98v-15.9A.663.663,0,0,1,19.393,16.75Z"
                                    transform="translate(-15.75 -16.75)"
                                    fill="#d5dbe1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21252"
                                  data-name="Group 21252"
                                  transform="translate(30.811 16.7)"
                                >
                                  <path
                                    id="Path_35045"
                                    data-name="Path 35045"
                                    d="M27.393,12.75h-2.98a.663.663,0,0,0-.662.662V34.607h2.98V13.412A.663.663,0,0,1,27.393,12.75Z"
                                    transform="translate(-23.75 -12.75)"
                                    fill="#00b871"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21253"
                                  data-name="Group 21253"
                                  transform="translate(10.236 9.375)"
                                >
                                  <path
                                    id="Path_35046"
                                    data-name="Path 35046"
                                    d="M0,0H15.924V1.987H0Z"
                                    transform="translate(0 11.258) rotate(-45)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21254"
                                  data-name="Group 21254"
                                  transform="translate(16.571 9.083)"
                                >
                                  <path
                                    id="Path_35047"
                                    data-name="Path 35047"
                                    d="M19.623,13.623H17.636V8.987H13V7h5.63a.994.994,0,0,1,.993.993Z"
                                    transform="translate(-13 -7)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21255"
                                  data-name="Group 21255"
                                  transform="translate(2 37.563)"
                                >
                                  <path
                                    id="Path_35048"
                                    data-name="Path 35048"
                                    d="M2,28.5H44.389v1.987H2Z"
                                    transform="translate(-2 -28.5)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21256"
                                  data-name="Group 21256"
                                  transform="translate(8.623 26.304)"
                                >
                                  <path
                                    id="Path_35049"
                                    data-name="Path 35049"
                                    d="M13.954,33.246H7.993A.994.994,0,0,1,7,32.253v-10.6A1.658,1.658,0,0,1,8.656,20h4.636a1.658,1.658,0,0,1,1.656,1.656v10.6A.994.994,0,0,1,13.954,33.246ZM8.987,31.259h3.974V21.987H8.987Z"
                                    transform="translate(-7 -20)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21257"
                                  data-name="Group 21257"
                                  transform="translate(19.22 21.005)"
                                >
                                  <path
                                    id="Path_35050"
                                    data-name="Path 35050"
                                    d="M21.954,34.545H15.993A.994.994,0,0,1,15,33.552v-15.9A1.658,1.658,0,0,1,16.656,16h4.636a1.658,1.658,0,0,1,1.656,1.656v15.9A.994.994,0,0,1,21.954,34.545Zm-4.967-1.987h3.974V17.987H16.987Z"
                                    transform="translate(-15 -16)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21258"
                                  data-name="Group 21258"
                                  transform="translate(29.818 15.706)"
                                >
                                  <path
                                    id="Path_35051"
                                    data-name="Path 35051"
                                    d="M29.954,35.844H23.993A.994.994,0,0,1,23,34.85V13.656A1.658,1.658,0,0,1,24.656,12h4.636a1.658,1.658,0,0,1,1.656,1.656V34.85A.994.994,0,0,1,29.954,35.844Zm-4.967-1.987h3.974V13.987H24.987Z"
                                    transform="translate(-23 -12)"
                                  ></path>
                                </g>
                              </g>
                            </g>
                          </svg>
                        </div>
                        <div class="flex flex-col">
                          <span class="swiper-slide-label">Giao dịch</span>
                          <span class="swiper-slide-text"
                            >Giao dịch và kiếm thêm lợi nhuận</span
                          >
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide">
                      <div class="swiper-slide-inner">
                        <div class="svg-winner">
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="80.96"
                            height="85.92"
                            viewBox="0 0 80.96 85.92"
                            class="swiper-slide-icon"
                          >
                            <g
                              id="Group_21272"
                              data-name="Group 21272"
                              transform="translate(-760.16 -2483.682)"
                            >
                              <g
                                id="页面1"
                                transform="translate(759.443 2483.682)"
                              >
                                <g
                                  id="_1_未登录"
                                  data-name="1_未登录"
                                  transform="translate(2.358 0)"
                                >
                                  <g id="编组-7" transform="translate(0)">
                                    <g id="编组备份">
                                      <g
                                        id="Group-6-Copy-4"
                                        transform="translate(0)"
                                      >
                                        <g
                                          id="分组-3"
                                          transform="translate(0 0)"
                                        >
                                          <path
                                            id="合并形状"
                                            d="M2.642,31.447a25.424,25.424,0,0,0,6.465.83A25.725,25.725,0,0,0,34.71,6.433V3.946H37.2A39.152,39.152,0,1,1-1.642,43.1,39.51,39.51,0,0,1-.4,33.232l.623-2.419Z"
                                            transform="translate(1.642 2.031)"
                                            fill="#fff"
                                            stroke="#fff"
                                            stroke-width="3.283"
                                            fill-rule="evenodd"
                                          ></path>
                                          <path
                                            id="_3"
                                            data-name="3"
                                            d="M18.81,5.345A6.25,6.25,0,0,1,15.39,11.2c1.38.791,1.976,1.929,1.976,4.016a7.46,7.46,0,0,1-7.4,7.685c-3.921,0-6.337-2.119-6.337-5.345a6.969,6.969,0,0,1,.094-1.138H7.548a4.394,4.394,0,0,0-.063.7,2.224,2.224,0,0,0,2.51,2.277c1.976,0,3.482-1.9,3.482-4.016a2.256,2.256,0,0,0-2.51-2.467h-.6l.659-3.415h.627a3.581,3.581,0,0,0,3.262-3.7,2.032,2.032,0,0,0-2.1-2.277,3.353,3.353,0,0,0-3.2,2.751H5.7C6.669,1.992,9.43,0,13.351,0A5.185,5.185,0,0,1,18.81,5.345Z"
                                            transform="translate(4.353 0)"
                                            fill="#fff"
                                          ></path>
                                        </g>
                                      </g>
                                    </g>
                                  </g>
                                </g>
                              </g>
                              <g id="money" transform="translate(778 2511.971)">
                                <g
                                  id="Group_21228"
                                  data-name="Group 21228"
                                  transform="translate(2.369 0.583)"
                                >
                                  <path
                                    id="Path_35009"
                                    data-name="Path 35009"
                                    d="M10.028,40.308c8.793,0,14.831,4.555,19.068,4.555s13.135-2.436,13.135-21.927S30.26.583,25.6.583C3.777.583-5.544,40.308,10.028,40.308Z"
                                    transform="translate(-2.29 -0.583)"
                                    fill="#efefef"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21229"
                                  data-name="Group 21229"
                                  transform="translate(7.085 7.469)"
                                >
                                  <path
                                    id="Path_35010"
                                    data-name="Path 35010"
                                    d="M7.907,9.814A1.907,1.907,0,1,1,9.814,7.907,1.909,1.909,0,0,1,7.907,9.814Zm0-2.542a.636.636,0,1,0,.636.636A.637.637,0,0,0,7.907,7.271Z"
                                    transform="translate(-6 -6)"
                                    fill="#a4afc1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21230"
                                  data-name="Group 21230"
                                  transform="translate(15.665 8.422)"
                                >
                                  <path
                                    id="Path_35011"
                                    data-name="Path 35011"
                                    d="M25.78,12.47c4.4,2.581,7.945,9.928,7.945,15.254,0,3.814-1.907,7.627-6.356,7.627H19.106c-4.449,0-6.356-3.814-6.356-7.627,0-5.326,3.547-12.674,7.945-15.254L16.538,9.013a1.27,1.27,0,0,1,.788-2.263H29.148a1.27,1.27,0,0,1,.788,2.263Z"
                                    transform="translate(-12.75 -6.75)"
                                    fill="#f3f3f1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21231"
                                  data-name="Group 21231"
                                  transform="translate(15.665 8.422)"
                                >
                                  <path
                                    id="Path_35012"
                                    data-name="Path 35012"
                                    d="M15.61,27.725c0-5.326,3.547-12.674,7.945-15.254L19.4,9.013a1.27,1.27,0,0,1,.788-2.263h-2.86a1.27,1.27,0,0,0-.788,2.263l4.157,3.458C16.3,15.051,12.75,22.4,12.75,27.725c0,3.814,1.907,7.627,6.356,7.627h2.86C17.517,35.352,15.61,31.538,15.61,27.725Z"
                                    transform="translate(-12.75 -6.75)"
                                    fill="#d5dbe1"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21232"
                                  data-name="Group 21232"
                                  transform="translate(18.018 7.469)"
                                >
                                  <path
                                    id="Path_35013"
                                    data-name="Path 35013"
                                    d="M25.709,13.419l-1.192-1.49,4.321-3.458a.3.3,0,0,0,.1-.352.291.291,0,0,0-.294-.212H16.824a.291.291,0,0,0-.294.212.3.3,0,0,0,.1.351l4.322,3.459-1.192,1.49L15.441,9.961A2.223,2.223,0,0,1,16.824,6H28.648a2.224,2.224,0,0,1,1.381,3.962Z"
                                    transform="translate(-14.601 -6)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21233"
                                  data-name="Group 21233"
                                  transform="translate(18.525 13.189)"
                                >
                                  <path
                                    id="Path_35014"
                                    data-name="Path 35014"
                                    d="M15,10.5H30.254v1.907H15Z"
                                    transform="translate(-15 -10.5)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21234"
                                  data-name="Group 21234"
                                  transform="translate(19.442 13.319)"
                                >
                                  <path
                                    id="Path_35015"
                                    data-name="Path 35015"
                                    d="M26.563,35.26H17.982a7.351,7.351,0,0,1-2.261-.338l.589-1.815a5.393,5.393,0,0,0,1.673.245h8.581c3.988,0,5.4-3.595,5.4-6.674,0-5.258-3.563-12.137-7.475-14.433l.966-1.644c4.482,2.633,8.415,10.144,8.415,16.078C33.872,30.95,31.612,35.26,26.563,35.26Z"
                                    transform="translate(-15.721 -10.602)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21235"
                                  data-name="Group 21235"
                                  transform="translate(22.975 22.088)"
                                >
                                  <path
                                    id="Path_35016"
                                    data-name="Path 35016"
                                    d="M22.161,26.4H18.5V24.492h3.661a.795.795,0,0,0,0-1.589h-.966a2.7,2.7,0,0,1,0-5.4h3.661v1.907H21.195a.795.795,0,0,0,0,1.589h.966a2.7,2.7,0,0,1,0,5.4Z"
                                    transform="translate(-18.5 -17.5)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21236"
                                  data-name="Group 21236"
                                  transform="translate(25.199 20.181)"
                                >
                                  <path
                                    id="Path_35017"
                                    data-name="Path 35017"
                                    d="M20.25,16h1.907v3.814H20.25Z"
                                    transform="translate(-20.25 -16)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21237"
                                  data-name="Group 21237"
                                  transform="translate(25.199 29.079)"
                                >
                                  <path
                                    id="Path_35018"
                                    data-name="Path 35018"
                                    d="M20.25,23h1.907v3.814H20.25Z"
                                    transform="translate(-20.25 -23)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21238"
                                  data-name="Group 21238"
                                  transform="translate(2 36.071)"
                                >
                                  <path
                                    id="Path_35019"
                                    data-name="Path 35019"
                                    d="M2,28.5H42.678v1.907H2Z"
                                    transform="translate(-2 -28.5)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21239"
                                  data-name="Group 21239"
                                  transform="translate(8.038 18.592)"
                                >
                                  <path
                                    id="Path_35020"
                                    data-name="Path 35020"
                                    d="M14.695,17.928h3.814V14.75H14.059a7.309,7.309,0,1,0,0,14.619h4.449V26.191H14.695a4.131,4.131,0,1,1,0-8.263Z"
                                    transform="translate(-6.75 -14.75)"
                                    fill="#2fdf84"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21240"
                                  data-name="Group 21240"
                                  transform="translate(8.038 18.592)"
                                >
                                  <path
                                    id="Path_35021"
                                    data-name="Path 35021"
                                    d="M9.61,22.059A7.309,7.309,0,0,1,16.92,14.75h-2.86a7.309,7.309,0,1,0,0,14.619h2.86A7.309,7.309,0,0,1,9.61,22.059Z"
                                    transform="translate(-6.75 -14.75)"
                                    fill="#00b871"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21241"
                                  data-name="Group 21241"
                                  transform="translate(7.085 17.639)"
                                >
                                  <path
                                    id="Path_35022"
                                    data-name="Path 35022"
                                    d="M19.03,30.525H14.263a8.263,8.263,0,1,1,0-16.525H19.03a.954.954,0,0,1,.953.953v3.178a.954.954,0,0,1-.953.953H14.9a3.178,3.178,0,1,0,0,6.356H19.03a.954.954,0,0,1,.953.953v3.178A.954.954,0,0,1,19.03,30.525ZM14.263,15.907a6.356,6.356,0,0,0,0,12.712h3.814V27.347H14.9a5.085,5.085,0,0,1,0-10.17h3.178V15.907Z"
                                    transform="translate(-6 -14)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21242"
                                  data-name="Group 21242"
                                  transform="translate(15.347 18.592)"
                                >
                                  <path
                                    id="Path_35023"
                                    data-name="Path 35023"
                                    d="M12.5,14.75h1.907v3.178H12.5Z"
                                    transform="translate(-12.5 -14.75)"
                                  ></path>
                                </g>
                                <g
                                  id="Group_21243"
                                  data-name="Group 21243"
                                  transform="translate(15.347 30.033)"
                                >
                                  <path
                                    id="Path_35024"
                                    data-name="Path 35024"
                                    d="M12.5,23.75h1.907v3.178H12.5Z"
                                    transform="translate(-12.5 -23.75)"
                                  ></path>
                                </g>
                              </g>
                            </g>
                          </svg>
                        </div>
                        <div class="flex flex-col">
                          <span class="swiper-slide-label">Thắng Giải</span>
                          <span class="swiper-slide-text"
                            >Bạn càng giao dịch nhiều thì bạn càng <br />
                            có nhiều cơ hội nhận giải</span
                          >
                        </div>
                      </div>
                    </div>
                  </div>
                  <span
                    class="swiper-notification"
                    aria-live="assertive"
                    aria-atomic="true"
                  ></span>
                </div>
              </div>
              <div class="info-guide-content">
                <h4>Giới thiệu về Thử thách</h4>
                <p>
                  Thử Thách Liên Hoàn là một chương trình cho phép các nhà giao
                  dịch đạt được phần thưởng cho cả trận thua và thắng của họ.
                  Thử thách này bắt đầu từ một quỹ cộng đồng bao gồm 0,05% tổng
                  khối lượng giao dịch trên nền tảng của chúng tôi. Quỹ này sẽ
                  được sử dụng để thưởng cho người dùng đạt giải Độc đắc - 0,1%
                  tổng quỹ cộng đồng cho mỗi người thắng cuộc.
                </p>
                <h5>Các điều khoản và điều kiện</h5>
                <p>
                  Người dùng chỉ có thể giao dịch Tăng hoặc Giảm trong một phiên
                  và phải giao dịch trong các phiên liên tiếp nhau. Mỗi giao
                  dịch trong một phiên phải tối thiểu là $10. Số chuỗi thắng
                  hoặc số chuỗi thua phải là 9 lần. Trong trường hợp có nhiều
                  người dùng cùng trúng giải Độc Đắc trong cùng một phiên, giải
                  thưởng sẽ được chia đều cho tất cả người dùng trúng giải trong
                  cùng thời điểm. Người dùng phải hoàn thành xác minh danh tính
                  (KYC) trên hệ thống của chúng tôi để đủ điều kiện tham gia thử
                  thách này. Ngoài ra, hàng ngày sẽ có một giải Mega x5 giá trị
                  thưởng được chọn ngẫu nhiên từ những người dùng trúng giải Độc
                  Đắc trong ngày hiện tại. Nếu không có người dùng trúng giải
                  Độc Đắc nào trong ngày hiện tại thì cũng sẽ không có giải
                  Mega. Xin lưu ý rằng phần thưởng sẽ được chuyển vào Ví của tài
                  khoản sau khoảng 48 giờ.
                </p>
                <p>
                  Chúng tôi có quyền thay đổi/ sửa đổi bất kỳ thông tin trong
                  Điều khoản và Điều kiện hoặc bất kỳ chính sách/ hướng dẫn nào
                  của nền tảng của chúng tôi, vào bất cứ thời gian nào và theo
                  quyết định riêng của chúng tôi. Mọi thay đổi/ sửa đổi sẽ có
                  hiệu lực ngay sau khi thông tin điều chỉnh được đăng trên nền
                  tảng của chúng tôi hoặc sẽ thông báo cho bạn về những thay
                  đổi/ sửa đổi đó qua email.
                </p>
                <p>
                  Chương trình Thử Thách Liên Hoàn và các lợi ích của nó được
                  cung cấp theo quyết định riêng của chúng tôi. Chúng tôi có
                  quyền sửa đổi hoặc hủy bỏ giải Độc Đắc cũng như cách mà chúng
                  tôi đánh giá và trao thưởng cho các giao dịch đủ điều kiện của
                  bạn.
                </p>
                <p>
                  Chúng tôi có quyền sửa đổi hoặc chấm dứt quyền nhận giải Độc
                  Đắc của bạn khi phát hiện ra bất kỳ hành vi gian lận và không
                  trung thực nào do bạn thực hiện để nhận thêm giải theo ưu đãi
                  chương trình. Bạn sẽ bị tước quyền tham gia vào chương trình
                  Thử Thách Liên Hoàn hoặc bị cấm sử dụng vĩnh viễn dịch vụ của
                  chúng tôi mà sẽ không có thông báo trước đối với những trường
                  hợp như vậy. Quyết định của chúng tôi sẽ là quyết định cuối
                  cùng và chúng tôi sẽ không tham gia vào bất kỳ cuộc tranh luận
                  nào liên quan đến vấn đề này.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "HelloWorld",
  data() {
    return {
      msg: "Welcome to Your Vue.js App",
    };
  },
};
</script>

<style scoped>
@font-face {
  font-family: "Frizon";
  src:
    local("Frizon"),
    url("~@/assets/fonts/Frizon.ttf") format("truetype");
}

#main-content {
  width: 100%;
}

.community-jackpot {
  padding-top: 95px;
  background: #090b0d url("~@/assets/images/bg-1.bb570abf.png") top no-repeat;
  background-size: 100% auto;
  overflow: hidden;
  position: relative;
}

.community-jackpot {
  overflow: hidden;
  position: relative;
}

@media screen and (max-width: 767px) {
  .community-jackpot {
    background-position: center 125px;
    padding-top: 70px;
  }
}

.community-jackpot-title {
  font-family: "Frizon";
  font-size: 50px;
  line-height: 1;
  margin-bottom: 95px;
  color: #fff;
  text-align: center;
  padding: 0 20px;
}

@media (max-width: 767px) {
  .community-jackpot-title {
    font-size: 40px;
    line-height: 1.5;
  }
}

.community-jackpot .prize-pool {
  font-family: Frizon;
  color: #f7a62f;
  width: 520px;
  max-width: 90%;
  height: 116px;
  margin: 0 auto;
  position: relative;
  border: 1px solid #592ae6;
  border-radius: 25px;
  background: rgba(41, 12, 126, 0.44);
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  -webkit-box-pack: center;
  -ms-flex-pack: center;
  justify-content: center;
  -webkit-box-align: stretch;
  -ms-flex-align: stretch;
  align-items: stretch;
}

@media (max-width: 767px) {
  .community-jackpot .prize-pool {
    height: 82px;
  }
}

.community-jackpot .prize-pool:before {
  content: "";
  width: 108px;
  height: 108px;
  display: block;
  background-size: cover;
  background: url("~@/assets/images/box-2.cca970d1.png") 0 0 no-repeat;
  position: absolute;
  top: 50%;
  -webkit-transform: translateY(-50%);
  transform: translateY(-50%);
  left: -54px;
}

@media (max-width: 767px) {
  .community-jackpot .prize-pool:before {
    background-size: cover;
    width: 74px;
    height: 74px;
    left: -37px;
  }
}

.community-jackpot .prize-pool:after {
  content: "";
  width: 148px;
  height: 148px;
  display: block;
  background: url("~@/assets/images/box-3.892cbd71.png") 0 0 no-repeat;
  background-size: cover;
  position: absolute;
  top: 50%;
  -webkit-transform: translateY(-50%);
  transform: translateY(-50%);
  right: -74px;
}

@media (max-width: 767px) {
  .community-jackpot .prize-pool:after {
    background-size: cover;
    width: 98px;
    height: 98px;
    right: -49px;
  }
}

.community-jackpot .prize-pool-label {
  height: 48px;
  position: absolute;
  top: -32px;
  left: 50%;
  -webkit-transform: translateX(-50%);
  transform: translateX(-50%);
  font-size: 20px;
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
}

@media screen and (max-width: 767px) {
  .community-jackpot .prize-pool-label {
    font-size: 12px;
    top: -16px;
    height: 32px;
  }
}

.community-jackpot .prize-pool-label span {
  border-radius: 35px;
  border: 2px solid #592ae6;
  background: #290c7e;
  color: #f7a62f;
  position: relative;
  z-index: 2;
  padding: 0 20px;
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  -webkit-box-align: center;
  -ms-flex-align: center;
  align-items: center;
}

.community-jackpot .prize-pool-label:after {
  content: "";
  display: block;
  width: 100%;
  height: 75px;
  background: url("~@/assets/images/fire_2.0bb9224e.png") 0 0 no-repeat;
  background-size: 100% auto !important;
  position: absolute;
  left: 0;
  top: -35px;
  z-index: -1;
}

@media screen and (max-width: 767px) {
  .community-jackpot .prize-pool-label:after {
    content: "";
    display: block;
    width: 100%;
    height: 75px;
    background: url("~@/assets/images/fire_2.0bb9224e.png") 0 0 no-repeat;
    background-size: 100% auto !important;
    position: absolute;
    left: 0;
    top: -25px;
    z-index: -1;
  }
}

.community-jackpot .prize-pool-value {
  font-size: 48px;
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  -webkit-box-align: center;
  -ms-flex-align: center;
  align-items: center;
}

@media (max-width: 767px) {
  .community-jackpot .prize-pool-value {
    font-size: 30px;
  }
}

.community-jackpot .prize-pool-mega-fund {
  position: absolute;
  width: 370px;
  height: 77px;
  background: url("~@/assets/images/mega-fund.9f6d81da.png") 0 0 no-repeat;
  background-size: cover !important;
  left: 50%;
  -webkit-transform: translateX(-50%);
  transform: translateX(-50%);
  bottom: -60px;
  color: #b100ff;
  font-size: 26px;
  font-weight: 700;
  text-align: center;
  padding-top: 9px;
}

@media (max-width: 767px) {
  .community-jackpot .prize-pool-mega-fund {
    background-size: cover;
    font-size: 18px;
    width: 250px;
    height: 52px;
    bottom: -46px;
    padding-top: 4px;
  }
}

.community-jackpot .go-trade {
  display: block;
  width: 220px;
  height: 50px;
  margin: 90px auto 25px;
  padding: 0 10px;
  line-height: 50px;
  background: #2a74f9;
  border-radius: 10px;
  font-family: Lato;
  font-size: 14px;
  font-weight: 900;
  color: #fff;
  text-align: center;
  font-family: Arial;
}

.community-jackpot .link-to-info {
  text-decoration: underline;
  color: #f7a62f;
  text-align: center;
  cursor: pointer;
}

.community-jackpot .bg {
  background: url("~@/assets/images/bg-2.b876c51a.png") top no-repeat;
  background-size: 100% auto !important;
  position: relative;
  z-index: 2;
  padding-top: 95px;
  margin-top: 70px;
}

@media (max-width: 767px) {
  .community-jackpot .bg {
    border-radius: 100px 100px 0 0;
    background: #100235;
  }
}

.latest-winner {
  padding-bottom: 75px;
}

.latest-winner-subtitle {
  color: hsla(0, 0%, 100%, 0.3);
  font-size: 16px;
  font-weight: 700;
  margin-bottom: 10px;
  text-align: center;
}

@media screen and (max-width: 767px) {
  .latest-winner .latest-winner-subtitle {
    font-size: 16px;
    margin-bottom: 10px;
  }
}

.latest-winner-title {
  font-family: Frizon;
  margin-bottom: 45px;
  color: #fff;
  font-size: 40px;
  line-height: 42px;
  height: 48px;
  text-align: center;
}

@media screen and (max-width: 767px) {
  .latest-winner .latest-winner-title {
    font-size: 32px;
    background: none;
    padding-left: 0;
  }
}

.latest-winner-mega {
  width: 458px;
  max-width: 90%;
  height: 104px;
  margin: 0 auto 90px;
  padding-top: 28px;
  position: relative;
  border: 2px solid #f7a62f;
  border-radius: 27px;
  background: #020024;
  background: -webkit-gradient(
    linear,
    left top,
    left bottom,
    from(#020024),
    color-stop(0, #7a4900),
    to(#342500)
  );
  background: linear-gradient(180deg, #020024, #7a4900 0, #342500);
  text-align: center;
}

.latest-winner-mega:before {
  content: "";
  width: 99px;
  height: 99px;
  background: url("~@/assets/images/box-8.b7415b73.png") 0 0 no-repeat;
  background-size: cover !important;
  position: absolute;
  top: 0;
  left: 0;
  -webkit-transform: translateX(-50%);
  transform: translateX(-50%);
}

@media (max-width: 767px) {
  .latest-winner-mega:before {
    width: 60px;
    height: 60px;
  }
}

.latest-winner-mega:after {
  content: "";
  width: 93px;
  height: 93px;
  background: url("~@/assets/images/box-9.a5494ca0.png") 0 0 no-repeat;
  background-size: cover !important;
  position: absolute;
  top: 50%;
  right: 0;
  -webkit-transform: translate(50%, -50%);
  transform: translate(50%, -50%);
}
@media (max-width: 767px) {
  .latest-winner-mega:after {
    width: 60px;
    height: 60px;
  }
}

.latest-winner-mega-title {
  height: 24px;
  line-height: 22px;
  padding: 0 15px;
  margin: 0;
  font-size: 10px;
  font-weight: 700;
  color: #f7a62f;
  text-transform: uppercase;
  background: linear-gradient(45deg, #020024, #2c1d01 0, #2b1c01);
  border-radius: 7px;
  border: 1px solid #f7a62f;
  position: absolute;
  top: -12px;
  left: 50%;
  -webkit-transform: translateX(-50%);
  transform: translateX(-50%);
}

.latest-winner-mega-name {
  font-size: 24px;
  font-weight: 700;
  color: #f7a62f;
  text-transform: uppercase;
  margin-bottom: -5px;
  margin-top: -10px;
}

.latest-winner-mega-text {
  font-size: 14px;
  color: #fff;
  margin-bottom: 0;
}

.latest-winner-mega-prize {
  width: 298px;
  height: 66px;
  padding-top: 15px;
  background: url("~@/assets/images/mega-fund-2.bf69f480.png") 0 0 no-repeat;
  background-size: cover !important;
  font-size: 19px;
  font-weight: 700;
  color: #b100ff;
  position: absolute;
  bottom: -57px;
  left: 50%;
  -webkit-transform: translateX(-50%);
  transform: translateX(-50%);
}

@media (max-width: 767px) {
  .latest-winner-mega-prize {
    width: 250px;
    height: 52px;
    bottom: -46px;
    padding-top: 8px;
  }
}

.latest-winner-table {
  margin-bottom: 25px;
  border: 1px solid #1f1049;
  border-radius: 10px;
  overflow: hidden;
}

@media screen and (max-width: 767px) {
  .latest-winner .latest-winner-table {
    display: block;
    border: none;
    font-size: 12px;
    overflow: visible;
  }
}

.nav {
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  -ms-flex-wrap: wrap;
  flex-wrap: wrap;
  padding-left: 0;
  margin-bottom: 0;
  list-style: none;
}

.nav-tabs {
  border-radius: 10px 10px 0 0;
  background: #12023e;
  border-bottom: none;
}

@media screen and (max-width: 767px) {
  .nav-tabs {
    background: none;
    border-bottom: 1px solid hsla(0, 0%, 100%, 0.14);
    margin: 0 -15px 28px;
    padding: 0 32px;
  }
}

.nav-tabs .item {
  padding: 16px 44px;
  position: relative;
  color: #fff;
  font-size: 16px;
  font-weight: 700;
  cursor: pointer;
}

@media screen and (max-width: 767px) {
  .nav-tabs .item {
    padding: 16px;
    width: 50%;
    text-align: center;
  }
}

.nav-tabs .item.active:before {
  content: "";
  height: 4px;
  width: 160px;
  border-radius: 2px;
  background: #00b6ff;
  position: absolute;
  left: 50%;
  -webkit-transform: translateX(-50%);
  transform: translateX(-50%);
  bottom: 5px;
}

@media screen and (max-width: 767px) {
  .nav-tabs .item.active:before {
    bottom: 0;
  }
}

.table-winner {
  display: flex;
  flex-direction: column;
}

.latest-winner-item--header {
  background: #1f1049;
  color: hsla(0, 0%, 100%, 0.3);
  display: flex;
  justify-content: space-between;
  width: 100%;
  border: none;
}

.latest-winner-item.latest-winner-item--header > th {
  border: none;
  padding: 15px 25px;
}

.latest-winner-items {
  display: flex;
  justify-content: space-between;
  width: 100%;
  border-bottom: 1px solid rgba(200, 200, 200, 0.9);
  padding: 15px 25px;
  background: #12023e;
  font-weight: 600;
}

.latest-winner-items .win {
  color: #18c67a;
}

.latest-winner-items .loss {
  color: #fc5f5f;
}

.justify-content-center {
  -webkit-box-pack: center !important;
  -ms-flex-pack: center !important;
  justify-content: center !important;
}

.d-flex {
  display: -webkit-box !important;
  display: -ms-flexbox !important;
}

.paging {
  padding: 15px 0;
}

.paging ul {
  height: 30px;
  display: flex;
}

.paging ul li {
  display: inline-block;
  margin: 0 10px;
}

.paging ul li a.active {
  border-radius: 50%;
  background: #2a74f9;
}

.paging ul li a.active {
  color: #fff !important;
}
.paging ul li a.active {
  color: #fff !important;
}
.paging ul li a {
  color: #fff;
  display: block;
  width: 30px;
  height: 30px;
  line-height: 30px;
  text-decoration: none;
}
.paging ul li a.active {
  color: #fff !important;
}
.btn-active-paging {
  color: #080d1f !important;
}
a:not(.md-button) {
  text-decoration: none;
}
a:not(.md-button) {
  transition: 0.3s cubic-bezier(0.25, 0.8, 0.25, 1);
  transition-property: color, background-color, opacity;
}
.btn-active-paging {
  color: #080d1f !important;
}
.btn-active-paging {
  color: #080d1f !important;
}

.community-jackpot .info-guide {
  color: #fff;
  padding-bottom: 30px;
  background: url("~@/assets/images/bg-3.805f565a.png") top no-repeat;
  background-size: cover;
  position: relative;
}

@media screen and (max-width: 767px) {
  .community-jackpot .info-guide {
    padding-bottom: 15px;
  }
}

.community-jackpot .info-guide-inner {
  padding-top: 100px;
}

@media screen and (max-width: 767px) {
  .community-jackpot .info-guide .info-guide-inner {
    padding-top: 55px;
  }
}

.community-jackpot .info-guide:after {
  content: "";
  display: block;
  width: 318px;
  height: 318px;
  background: url("~@/assets/images/streak-icon-3.7d63b463.png");
  background-size: cover !important;
  position: absolute;
  top: 70px;
  right: -70px;
}

@media (max-width: 767px) {
  .community-jackpot .info-guide:after {
    width: 174px;
    height: 174px;
    right: -55px;
    top: -70px;
  }
}

@media screen and (max-width: 767px) {
  .community-jackpot .info-guide-body {
    margin-right: -15px;
  }
}

.community-jackpot .info-guide-subtitle {
  color: #f7a62f;
  font-size: 17px;
  font-weight: 700;
  margin-bottom: 10px;
}
@media screen and (max-width: 767px) {
  .community-jackpot .info-guide-subtitle {
    font-size: 12px;
  }
}

.community-jackpot .info-guide-title {
  font-size: 40px;
  font-weight: 700;
  font-family: Frizon;
  line-height: 1;
  margin-bottom: 20px;
}

@media screen and (max-width: 767px) {
  .community-jackpot .info-guide-title {
    font-size: 34px;
    line-height: 45px;
    margin-bottom: 15px;
  }
}

@media screen and (max-width: 767px) {
  .community-jackpot .info-guide-title {
    font-size: 34px;
    line-height: 45px;
    margin-bottom: 15px;
  }
}

.community-jackpot .info-guide .info-guide-slider {
  width: 100%;
  margin-bottom: 20px;
}

@media screen and (max-width: 767px) {
  .community-jackpot .info-guide .info-guide-slider {
    width: 620px;
  }
}

.swiper-container {
  margin-left: auto;
  margin-right: auto;
  position: relative;
  overflow: hidden;
  list-style: none;
  padding: 0;
  z-index: 1;
}

.swiper-community-jackpot {
  margin-top: 25px;
}

.community-jackpot .info-guide .swiper-community-jackpot .swiper-wrapper {
  -webkit-box-align: stretch;
  -ms-flex-align: stretch;
  align-items: stretch;
  display: flex;
  transition-duration: 0ms;
  transform: translate3d(0px, 0px, 0px);
}

.swiper-container .swiper-notification {
  position: absolute;
  left: 0;
  top: 0;
  pointer-events: none;
  opacity: 0;
  z-index: -1000;
}
.community-jackpot .info-guide .swiper-community-jackpot .swiper-slide {
  padding-right: 20px;
  cursor: pointer;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  height: auto;
  width: 33.33%;
}
.swiper-slide {
  -webkit-flex-shrink: 0;
  -ms-flex-negative: 0;
  flex-shrink: 0;
  width: 100%;
  height: 100%;
  position: relative;
  -webkit-transition-property: -webkit-transform;
  transition-property: -webkit-transform;
  -o-transition-property: transform;
  transition-property: transform;
  transition-property:
    transform,
    -webkit-transform;
  display: flex;
  justify-content: space-between;
}

.swiper-slide-inner {
  padding: 15px 0;
  text-align: center;
}

@media screen and (max-width: 767px) {
  .community-jackpot .info-guide .swiper-community-jackpot .swiper-slide {
    width: 100%;
    padding-right: 15px;
  }

  .swiper-slide-inner {
    display: flex;
    flex-direction: row;
    text-align: left;
    align-items: center;
  }

  .swiper-slide {
    padding: 0;
  }

  .swiper-wrapper {
    display: flex;
    flex-direction: column;
  }

  .svg-winner {
    margin-right: 30px;
  }
  .community-jackpot .info-guide-content {
    padding-right: 15px;
  }
}

.community-jackpot .info-guide-content h4 {
  font-weight: 700;
  font-size: 26px;
  margin-bottom: 20px;
}

.community-jackpot .info-guide-content p {
  font-size: 14px;
  line-height: 24px;
  margin-bottom: 30px;
}
</style>
